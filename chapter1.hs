...
-- be compiled... rather, just import the function definitions into the Haskell interpreter
-- (ghci) to test your functions.
--
...
---------------------------------------------------------------------------------
-- (SCHEME) new-if is a function (not a special form), and as Scheme uses applicative order
-- evaluation, the arguments of the function are always evaluated first, so the second
-- clause will continue to be evaluated, resulting in a loop.
-- (HASKELL) Haskell uses lazy evaluation, and as such, the function arguments will not be
-- evaluated unless needed, so the program will run successfully.
...
-- procedure. (In section 1.3.4 we will see how to implement Newton's method in general as
...
-- observation that (bn/2)2 = (b2)n/2, keep, along with the exponent n and the base b, an
-- additional state variable a, and define the state transformation in such a way that the
-- product a bn is unchanged from state to state. At the beginning of the process a is taken
-- to be 1, and the answer is given by the value of a at the end of the process. In general,
-- the technique of defining an invariant quantity that remains unchanged from state to
...
-- (HASKELL) It's is important to note at this time that haskell has two functions foldr and
-- foldl that will essentially apply accumulate over a LIST, not a range of two values. As
-- such, we will define accumulate as it appears here in haskell as follows:
...
-- additional predicate of one argument that specifies the filter. Write filtered-accumulate
...
-- Explain.
-----------------------------------------------------------------------------------
-- (SCHEME + HASKELL) With applicative order evaluation and lazy evaluation, the order of
...
-- a procedure that applies the original procedure twice. For example, if inc is a procedure
-- that adds 1 to its argument, then (double inc) should be a procedure that adds 2. What
-- value is returned by
-- (((double (double double)) inc) 5) --> 2^4 --> + 16 --> 21 (QED)
-- (NOTE) Whereas Scheme would use nested parentheses, haskell has specific function
-- operators, which will be used here:
...
-- 2nth power. Write a procedure that takes as inputs a procedure that computes f and a
-- positive integer n and returns the procedure that computes the nth repeated application
-- of f.
